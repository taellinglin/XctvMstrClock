...
var emission_strength := 1.0 setget set_emission_strength
var emission_color := Color(1.0, 0, 0) setget set_emission_color
...
func _process(delta):
	...
	rotate_y(rotation.z)
	rotate_x(rotation.y)
	set_text_size(rotation.x*72)
	set_emission_color(Color(rand_range(0,1), rand_range(0,1), rand_range(0,1)))
	set_text(numbers[randi() % 9] as String)
...
func set_font(value: Font) -> void:
	...
	if label:
		if font:
			...
			label.add_font_override("font", preload("res://font/Daemon.tres"))
		set_text(text)
